...
export default function App() {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        // Ensure data is correctly structured and has unique complaintId
        const fetchedUsers = response.data.data.map((user: any) => ({
          _id: user._id,
          lostFoundId: user.lostFoundId,
          name: user.name,
          email: user.email,
          status: user.status,
          reportDate: new Date(user.dateAndTime).toLocaleDateString(),
  ...
  const handleDelete = async (userId: string) => {
    try {
      ...
      await axios.delete(`http://localhost:5000/api/v1/lostfound/${userId}`, {
        headers: {
  ...
  const handleUpdate = async () => {
    if (selectedUser) {
      try {
        ...
        await axios.put(
          `http://localhost:5000/api/v1/lostfound/${selectedUser._id}`,
          {
            status: editStatus,
            comment: editComment,
          },
          ...
          <ModalHeader>
            {selectedUser && (
              <span className="text-xl font-semibold mb-1">
                {selectedUser.reportType} Details
              </span>
            )}
          </ModalHeader>
                ...
                <div className="flex flex-row gap-10">
                  <div className="flex flex-col gap-4 mt-5">
                    ...
                    <div className="flex justify-between gap-14">
                      ...
                      <p className="text-sm">
                        <strong>Report Type:</strong> {selectedUser.reportType}
                      </p>
                      <p className="text-sm">
                        <strong>Report Date:</strong> {selectedUser.reportDate}
                      </p>
                    </div>
                    <p className="text-sm">
