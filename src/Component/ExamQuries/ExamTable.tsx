...
export default function App() {
    ...
    const fetchUsers = async () => {
      try {
        ...
        const fetchedUsers = response.data.data.map((user: any) => ({
          ...
          type: user.queryType,
          status: user.status,
          submissionDate: new Date(user.submissionDate).toLocaleDateString(),
  ...
  const handleDelete = async (userId: string) => {
    try {
      const token = localStorage.getItem("token"); // Retrieve token from localStorage or cookies
      await axios.delete(`http://localhost:5000/api/v1/examqueries/${userId}`, {
        headers: {
          Authorization: `Bearer ${token}`, // Include token in headers
        },
      });
  ...
  const handleUpdate = async () => {
    if (selectedUser) {
      try {
        const token = localStorage.getItem("token"); // Retrieve token from localStorage or cookies
        await axios.put(
          `http://localhost:5000/api/v1/examqueries/${selectedUser._id}`,
          {
            status: editStatus,
            comment: editComment,
          },
              ...
              <>
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex justify-between gap-14">
                    ...
                    <p className="text-sm">
                      <strong>Course Code:</strong> {selectedUser.course}
                    </p>
                    <p className="text-sm">
                      <strong>Course Name:</strong> {selectedUser.courseName}
                    </p>
                  </div>
                  <div className="flex justify-start gap-14">
                    <p className="text-sm">
                      <strong>Query Type:</strong> {selectedUser.type}
                    </p>
                    <p className="text-sm">
                  ...
                  <div className="flex justify-start gap-14 mb-4">
                    <p className="text-sm">
                  ...
                  {selectedUser.file && (
                    <p className="text-sm">
                      <Button
                        href={`data:application/pdf;base64,${selectedUser.file}`}
                        download={`query_file ${selectedUser.id}.pdf`}
                        as={Link}
                        variant="flat"
                        radius="full"
                        color="secondary"
